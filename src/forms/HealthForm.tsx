...
import type { HealthFormData, ReportData } from "../types";
...
// Define the form state type explicitly without extending HealthFormData
...
const HealthForm: React.FC<HealthFormProps> = ({ onSubmit, onBack }) => {
  ...
  // Categorized Problem Options
  const problemOptions = [
    {
      category: "أمراض وتلوث",
      items: [
        { value: "disease", label: "تفشي المرض" },
        { value: "contamination", label: "تلوث الغذاء/الماء" },
        { value: "environmental", label: "خطر صحي بيئي" },
      ],
    },
    {
      category: "إصابات وحالات طارئة",
      items: [
        { value: "injury", label: "إصابات جماعية" },
        { value: "mental", label: "أزمة الصحة النفسية" },
      ],
    },
    {
      category: "أخرى",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6" dir="rtl">
      ...
      <FormField
        ...
        required
        placeholder="صف المشكلة الصحية بالتفصيل"
      />

      {/* Using the new CustomSelect component */}
      <CustomSelect
        label="نوع المشكلة الصحية"
        value={formData.problemType}
        onChange={(value) => handleChange("problemType", value)}
        options={problemOptions}
        required
        showOtherField={formData.problemType === "other"}
        onOtherChange={(value) => handleChange("otherProblemType", value)}
        otherPlaceholder="يرجى تحديد نوع المشكلة الصحية"
      />

      <FormField
        label="عدد الأشخاص المتأثرين"
        type="number"
        value={formData.affectedCount}
        onChange={(value) => handleChange("affectedCount", value)}
        min="0"
      />
